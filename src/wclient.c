...
error_exit_core_interface_create:
	return (struct uvr_wc_core_interface) { .iType = UVR_WC_INTERFACE_NULL, .wlDisplay = NULL, .wlRegistry = NULL,
...
void uvr_wc_destroy(struct uvr_wc_destroy *uvrwc)
{
	...
	if (uvrwc->uvr_wc_buffer.shmBufferObjects && uvrwc->uvr_wc_buffer.wlBufferHandles) {
		for (i = 0; i < uvrwc->uvr_wc_buffer.bufferCount; i++) {
			if (uvrwc->uvr_wc_buffer.shmBufferObjects[i].shmFd != -1)
				close(uvrwc->uvr_wc_buffer.shmBufferObjects[i].shmFd);
			if (uvrwc->uvr_wc_buffer.shmBufferObjects[i].shmPoolData)
				munmap(uvrwc->uvr_wc_buffer.shmBufferObjects[i].shmPoolData, uvrwc->uvr_wc_buffer.shmBufferObjects[i].shmPoolSize);
			if (uvrwc->uvr_wc_buffer.wlBufferHandles[i].buffer)
				wl_buffer_destroy(uvrwc->uvr_wc_buffer.wlBufferHandles[i].buffer);
		}
