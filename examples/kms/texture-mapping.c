 ...
 */
static const struct app_vertex_data meshData[2][4] = {
	{
...
static int
create_vk_texture_image (struct app_vk *app);
...
create_vk_texture_image (struct app_vk *app)
{
	...
	imageViewCreateInfo.imageViewComponents = (VkComponentMapping) { .r = 0, .g = 0, .b = 0, .a = 0 };
	// Which aspect of image to view (i.e VK_IMAGE_ASPECT_COLOR_BIT view color)
	imageViewCreateInfo.imageViewSubresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
	// Start mipmap level to view from (https://en.wikipedia.org/wiki/Mipmap)
	imageViewCreateInfo.imageViewSubresourceRange.baseMipLevel = 0;
	...
	if (!app->kmr_vk_image[textureImageIndex].imageHandles[imageTransferIndex].image && \
	    !app->kmr_vk_image[textureImageIndex].imageViewHandles[imageTransferIndex].view)
		return -1;
                       ...
                       VkExtent2D extent2D)
{
	...
	for (uint32_t mesh = 0; mesh < MAX_SCENE_OBJECTS; mesh++) {
		model = (struct app_uniform_buffer_scene_model *) \
