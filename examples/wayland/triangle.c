...
struct app_vk {
	...
	/*
	 * 1. CPU visible transfer buffer [Used as primary buffer if physical device CPU/INTEGRATED]
...
int create_vk_framebuffers(struct app_vk *app, VkExtent2D extent2D)
{
	uint8_t framebufferCount = app->uvr_vk_image.imageCount;
	struct uvr_vk_framebuffer_images framebufferImages[framebufferCount];

	for (uint8_t i = 0; i < framebufferCount; i++) {
		framebufferImages[i].imageAttachments[0] = app->uvr_vk_image.imageViewHandles[i].view; // VkImageView->VkImage for color image attachment
	}
